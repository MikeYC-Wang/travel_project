...
# 引入 dotenv 與「最新版」的 Google Gemini 套件
import dotenv
...
# 升級：單日 AI 重新生成需要的資料
class AISingleDayRequest(BaseModel):
    destination: str
    notes: Optional[str] = None
...
async def generate_ai_itinerary(req: AIGenerateRequest, db: Session = Depends(get_db)):
    try:
        ...
        # 將選填項目加入 Prompt
        arrival_info = f"旅客預計抵達機場的時間為 {req.arrival_time}，請據此安排第一天的行程起點。" if req.arrival_time else ""
        notes_info = f"旅客的特別備註/需求：{req.notes}。請務必將此需求納入考量。" if req.notes else ""
        ...
        # 💡 先刪除子表 (activities)，再刪除主表 (day_plans) 以避免外鍵衝突
...
# 2. 單日 AI 重新生成
@app.post("/api/itinerary/days/{day_id}/ai-regenerate")
async def regenerate_single_day(day_id: int, req: AISingleDayRequest, db: Session = Depends(get_db)):
    try:
        day = db.query(DayPlanDB).filter(DayPlanDB.id == day_id).first()
        if not day:
            raise HTTPException(status_code=404, detail="找不到該天行程")
            
        notes_info = f"旅客對於這一天的特別要求：{req.notes}。" if req.notes else ""
        
        prompt = f"""
        你是一位專業的旅遊規劃師。請為我重新規劃【{req.destination}】第 {day.day_number} 天的「單日」旅遊行程。
        {notes_info}
        行程必須合理，包含 3 到 4 個活動。
        
        請嚴格遵守以下規則：
        1. 絕對不要回傳 Markdown 語法 (如 ```json)。
        2. 絕對不要有任何問候語。
        3. 直接回傳一個合法的 JSON 陣列 (僅包含該天的活動)，格式如下：
        [
            {{"time": "09:00", "title": "活動標題", "location": "地點名稱"}}
        ]
        """
        ...
        # 先刪除該天舊的活動
        db.query(ActivityDB).filter(ActivityDB.day_id == day_id).delete()
